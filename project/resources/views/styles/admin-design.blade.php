...
.col-sm-6 p{
    ...
    background: {{$gs->colors == null ? 'rgba(0, 0, 0, 0.70)':$gs->colors.'c2'}};
    }
.sidebar-menu-body {
...
#sidebar-menu ul.components ul li a:hover {
    {{--background-color:{{$gs->colors == null ? 'rgb(165, 27, 27,0.9)':$gs->colors.'66'}};--}}
    background: {{$gs->colors == null ? 'rgba(0, 0, 0, 0.70)':$gs->colors.'c2'}};
}
...
.dataTables_wrapper .dataTables_filter input.form-control:focus {
    border-bottom: 1px solid {{$gs->colors == null ? '#D75357':$gs->colors}};
}
</style>
